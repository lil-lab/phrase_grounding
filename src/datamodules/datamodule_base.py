...
def get_pretrained_tokenizer(from_pretrained, model_cls: str):
    if torch.distributed.is_initialized():
        if torch.distributed.get_rank() == 0:
            if model_cls == "vilt_aligner" or model_cls == "vilt_aligner_probe":
                BertTokenizerFast.from_pretrained(
                    from_pretrained, do_lower_case="uncased" in from_pretrained
                )
            elif model_cls == "mdetr":
...
class BaseDataModule(LightningDataModule):
    def __init__(self, _config):
        ...
        self.eval_batch_size = self.batch_size if _config["per_gpu_batchsize_eval"] == 0 else _config["per_gpu_batchsize_eval"]
        ...
        self.tokenizer = get_pretrained_tokenizer(tokenizer, _config["model_cls"])
        for tk in _config["additional_tokens"]:
            self.tokenizer.add_tokens(tk)
