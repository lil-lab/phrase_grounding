...
class EMA(pl.Callback):
    ...
    Implementation detail:
        ...
        - When multi gpu, we broadcast ema weights and the original weights in order to only hold 1 copy in memory.
          ...
          performance.
    """
    def __init__(self, decay: float = 0.9998, ema_device: Optional[Union[torch.device, str]] = None, pin_memory=True):
        super().__init__()
        self.decay = decay
    ...
    @rank_zero_only
    def on_before_zero_grad(self, trainer: "pl.Trainer", pl_module: pl.LightningModule, *args, **kwargs) -> None:
        # Resume training breaks
        # Update EMA weights
        with torch.no_grad():
    ...
    def on_validation_start(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        if pl_module.hparams.config["num_gpus"] > 4:
            print("Not using ema model during validation to avoid GPU memory overflow.")
        else:
            ...
            #print(torch.sum(torch.tensor([torch.sum(p) for p in self.original_state_dict.values()])))
            print(pl_module.global_rank, "The model is about to initialized !!")
            ...
            if pl_module.global_rank > 0:
                ...
                self.ema_state_dict = {}
            # TODO: clean the lines below
            # print(torch.sum(torch.tensor([torch.sum(p) for p in self.get_state_dict(pl_module).values()])))
    ...
    def on_test_start(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ckpt = torch.load(pl_module.hparams.config["load_path"], map_location="cpu")
        state_dict = ckpt["callbacks"]["EMA"]["ema_state_dict"]
        self._ema_state_dict_ready = ckpt["callbacks"]["EMA"]["_ema_state_dict_ready"]
        self.ema_state_dict = ckpt["callbacks"]["EMA"]["ema_state_dict"]

        if not self._ema_state_dict_ready:
            return  # Skip Lightning sanity validation check if no ema weights has been loaded from a checkpoint.

        print("EMA model is loaded for testing.")
        self.original_state_dict = deepcopy(self.get_state_dict(pl_module))
        # TODO: clean the lines below
        #print("validation started")
        #print(torch.sum(torch.tensor([torch.sum(p) for p in self.original_state_dict.values()])))
